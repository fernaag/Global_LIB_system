...
class Classification(Obj):
    ...
    def __init__(
        ...
        Obj.__init__(
            self, Name=Name, ID=ID, UUID=UUID
...
class MFAsystem(Obj):
    ...
    def IndexTableCheck(self):
        ...
        for indx in self.IndexTable.index:
            if (
                self.IndexTable.ix[indx]["Dimension"]
                != self.IndexTable.ix[indx]["Classification"].Dimension
            ):
                raise ValueError(
    ...
    def Flow_Sum_By_Element(self, FlowKey):
        ...
        return np.einsum(
            ...
            + "->"
            + self.IndexTable.ix["Time"].IndexLetter
            + self.IndexTable.ix["Element"].IndexLetter,
