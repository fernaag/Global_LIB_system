...
def ParseModelControl(Model_Configsheet, ScriptConfig):
    ...
    while len(Model_Configsheet.cell_value(SCix, 3)) > 0:
        ScriptConfig[
            Model_Configsheet.cell_value(SCix, 2)
...
def ParseClassificationFile_Main(Classsheet, Mylog):
    ...
    while True:
        ...
        try:
            ...
            TheseItems.append(
                Classsheet.cell_value(ri, ci)
            )  # read the first classification item
        except:
            Mylog.info(
                "End of file or formatting error while reading the classification file in column "
                + str(ci)
                + ". Check if all classifications are present. If yes, you are good to go!"
            )
            break
        while True:
            ri += 1
...
def ParseConfigFile(Model_Configsheet, ScriptConfig, Mylog):
    ...
    while True:
        if len(Model_Configsheet.cell_value(PLix, 2)) > 0:
            ...
            PL_IndexLayer.append(
                ListStringToListNumbers(Model_Configsheet.cell_value(PLix, 7))
    ...
    while True:
        if Model_Configsheet.cell_value(PrLix, 2) != "":
            try:
                ScriptConfig[
                    Model_Configsheet.cell_value(PrLix, 2)
...
def ReadParameter(
    ...
    if ParHeader.cell_value(ri, 1) == "List":
        ...
        for m in range(0, len(ThisParIx)):
            ...
            IndexSizesM.append(
                IndexTable.set_index("IndexLetter").ix[ThisDim]["IndexSize"]
            )
